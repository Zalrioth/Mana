...
void model_process_node(struct Model* model, struct aiNode* node, const struct aiScene* scene) {
  for (unsigned int mesh_num = 0; mesh_num < node->mNumMeshes; mesh_num++) {
    ...
    vector_push_back(model->meshes, model_process_mesh(model, mesh, scene));
  }
...
struct Mesh* model_process_mesh(struct Model* model, struct aiMesh* mesh, const struct aiScene* scene) {
  ...
  model_load_material_textures(new_mesh->textures, material, aiTextureType_DIFFUSE, "texture_diffuse");
  model_load_material_textures(new_mesh->textures, material, aiTextureType_SPECULAR, "texture_specular");
  model_load_material_textures(new_mesh->textures, material, aiTextureType_HEIGHT, "texture_normal");
  model_load_material_textures(new_mesh->textures, material, aiTextureType_AMBIENT, "texture_height");
