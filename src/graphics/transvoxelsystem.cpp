...
TransvoxelSystem::TransvoxelSystem(int size) : size(size) {
  Voxel example;
  volumeData.resize(size * size * size, example);
  setupVolumeData();

  this->volumeShader = new Shader("volume.vert", "volume.frag");
}
...
glm::vec3 TransvoxelSystem::gradientForPoint(int x, int y, int z) {
  ...
  float gradx = (voxelForVolumePos(maxx, y, z).isovalue -
                 voxelForVolumePos(minx, y, z).isovalue) /
                divisorx;
  ...
  float grady = (voxelForVolumePos(x, maxy, z).isovalue -
                 voxelForVolumePos(x, miny, z).isovalue) /
                divisory;
  ...
  float gradz = (voxelForVolumePos(x, y, maxz).isovalue -
                 voxelForVolumePos(x, y, minz).isovalue) /
                divisorz;
...
glm::vec3 TransvoxelSystem::interpolateTransNormal(int x, int y, int z,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    int cornerOffset[3] = {0};
    cornerOffset[0] =
        transCornerOffsets[corner][0] * voxelScale /
        2;  // Divide by 2 because offset table uses values 2x as large
...
void TransvoxelSystem::computeTrianglesForVoxel(int x, int y, int z) {
  ...
  int cornerIndices[8] = {
      (x - voxelScale) * size * size + (y - voxelScale) * size +
          (z - voxelScale),
  ...
  float isorange = 0;
  if ((volumeData[cornerIndices[0]].isovalue) <= isorange) cubeIndex |= 1;
  if ((volumeData[cornerIndices[1]].isovalue) <= isorange) cubeIndex |= 2;
  if ((volumeData[cornerIndices[2]].isovalue) <= isorange) cubeIndex |= 4;
  if ((volumeData[cornerIndices[3]].isovalue) <= isorange) cubeIndex |= 8;
  if ((volumeData[cornerIndices[4]].isovalue) <= isorange) cubeIndex |= 16;
  if ((volumeData[cornerIndices[5]].isovalue) <= isorange) cubeIndex |= 32;
  if ((volumeData[cornerIndices[6]].isovalue) <= isorange) cubeIndex |= 64;
  if ((volumeData[cornerIndices[7]].isovalue) <= isorange) cubeIndex |= 128;
  ...
  for (int i = 0; i < cellData.GetVertexCount(); i++) {
    ...
    if (lodSmoothing) {
      ...
      while (lod > 1) {
        ...
        int centerVoxelIndex = (int)centerVoxelPosition[0] * size * size +
                               (int)centerVoxelPosition[1] * size +
                               (int)centerVoxelPosition[2];
...
void TransvoxelSystem::computeTrianglesForTransvoxel(int x, int y, int z) {
  ...
  int transCornerIndices[13] = {
      (x - voxelScale) * size * size + (y - voxelScale) * size +
          (z - voxelScale),
      (x - voxelScale / 2) * size * size + (y - voxelScale) * size +
          (z - voxelScale),
      (x)*size * size + (y - voxelScale) * size + (z - voxelScale),
      (x - voxelScale) * size * size + (y - voxelScale / 2) * size +
          (z - voxelScale),
      (x - voxelScale / 2) * size * size + (y - voxelScale / 2) * size +
          (z - voxelScale),
  ...
  for (int i = 0; i < transCellData.GetVertexCount(); i++) {
    ...
    glm::vec3 normal =
        interpolateTransNormal(x, y, z, isorange, corner1, corner2);
...
void TransvoxelSystem::drawVolumeData(EngineSettings *engineSettings,
                                      float aspect) {
  ...
  modelMatrix = glm::translate(
      modelMatrix, glm::vec3(-size / 2.0f, -size / 2.0f, -size / 2.0f));
