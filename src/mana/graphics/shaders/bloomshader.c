#include "mana/graphics/shaders/bloomshader.h"
/*
int bloom_shader_init(struct BloomShader* bloom_shader, struct VulkanState* vulkan_renderer, VkRenderPass render_pass, int descriptors) {
  bloom_shader->bloom_shader = calloc(1, sizeof(struct Shader));
  ...
  if (vkCreateDescriptorSetLayout(vulkan_renderer->device, &layout_info, NULL, &bloom_shader->shader->descriptor_set_layout) != VK_SUCCESS)
    return 0;
  ...
  if (vkCreateDescriptorPool(vulkan_renderer->device, &pool_info, NULL, &bloom_shader->shader->descriptor_pool) != VK_SUCCESS) {
  ...
  shader_init(blit_shader->shader, vulkan_renderer, "./assets/shaders/spirv/screenspace.vert.spv", "./assets/shaders/spirv/blit.frag.spv", NULL, vertex_input_info, render_pass, color_blending, VK_FRONT_FACE_CLOCKWISE, VK_FALSE, VK_SAMPLE_COUNT_1_BIT, false);
bloom_shader
  return 1;
}
