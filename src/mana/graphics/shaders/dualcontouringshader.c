...
int dual_contouring_shader_init(struct DualContouringShader* dual_countouring_shader, struct VulkanState* vulkan_renderer) {
  ...
  if (vkCreateDescriptorPool(vulkan_renderer->device, &poolInfo, NULL, &dual_countouring_shader->shader.descriptor_pool) != VK_SUCCESS) {
  ...
  shader_init(&dual_countouring_shader->shader, vulkan_renderer, "./assets/shaders/spirv/dualcontouring.vert.spv", "./assets/shaders/spirv/dualcontouring.frag.spv", NULL, vertex_input_info, vulkan_renderer->gbuffer->render_pass, color_blending, VK_FRONT_FACE_CLOCKWISE, VK_TRUE, vulkan_renderer->msaa_samples, true);
