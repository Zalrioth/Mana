#include "mana/graphics/entities/drawsprite.h"
...
int draw_sprite_init(struct DrawSprite* blit_sprite, struct VulkanRenderer* vulkan_renderer, struct Shader* shader) {
  ...
  buffer_info.buffer = blit_sprite->sprite.uniform_buffer;
  ...
  dcs[0].dstSet = blit_sprite->sprite.descriptor_set;
  dcs[0].dstBinding = 0;
  ...
  dcs[1].dstSet = blit_sprite->sprite.descriptor_set;
  dcs[1].dstBinding = 1;
