...
//nu
//https://github.com/orthecreedence/ghostie/blob/master/opengl/glsl/dof.bokeh.2.4.frag
// Pentagons look great here
//http://devlog-martinsh.blogspot.com/2011/11/glsl-depth-of-field-with-bokeh-v21.html
//https://forum.facepunch.com/f/games/uhjw/Bokeh-DoF-Shader/1/
// Think about this by using geometry shader to do pseudo bokeh
//https://mynameismjp.wordpress.com/2011/02/28/bokeh/
...
int samples = 4; // 5 samples on the first ring
int rings = 4; // 14 ring count
...
float gain = 0.6; // 1.0 highlight gain;

float bias = 0.5; // 0.25 bokeh edge bias
...
bool pentagon = false; // true use pentagon as bokeh shape?
float feather = 3.0; // 3.0 pentagon shape feather
...
void main()
{
	...
	float depth = linearize(texture(depthTex,vTexCoord.xy).x);

	if (depthblur)
	{
		depth = linearize(bdepth(vTexCoord.xy));
	}
	...
	float fDepth = linearize(focalDepth);
	...
	{
		col = texture(renderTex, vTexCoord.xy).rgb;
	}
