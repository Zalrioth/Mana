#version 410 core
//https://github.com/lragnarsson/Emerald-Engine/blob/master/src/shaders/grass_lod1.geom
...
const float GRASS_SCALE = 0.5f; // Uniform scale for grass
//const float MAGNITUDE = 0.5f; // Height scale for grass
...
void generate_grass(vec2 texCoord, vec3 fragPos, vec3 inNormal, vec3 push, vec3 base_1, vec3 base_2, const float[7] grass_x, const float[7] grass_y)
{
...
void main()
{
    ...
    vec3 center_triangle = gs_in[0].FragPos + 0.5 * frag_pos_base_01 +0.5 * frag_pos_02 + MAGNITUDE * GRASS_SCALE * GRASS_2_Y[6] * normal;
    ...
    for (int i = 0; i <= N_GRASS_STRAWS; i++) {
        ...
        if (gs_in[0].groundNormals.g < 0.5) {
            if (tall)
                generate_grass(tex_coord, frag_pos, normal, total_push, frag_pos_base_01, frag_pos_base_02, GRASS_2_X, GRASS_2_Y);
            else
                generate_grass(tex_coord, frag_pos, normal, total_push, frag_pos_base_01, frag_pos_base_02, GRASS_3_X, GRASS_3_Y);
        }
